...
primitive type의 변수들은 stack 영역에 저장이 되고 reference type은 heap에 저장이 된다. 왜냐하면 reference 들은 크기가 너무 커서 stack에 넣기 힘들기 때문이다.
![](images/2022-07-18-21-21-19.png)
primitive 변수들은 할당 연산자를 사용해 같은 값을 할당 시키면 처음에는 0001로 같은 주소를 가리키다가 변수의 내용이 바뀌면 새로운 주소를 할당받아 그 값도 바꾼다. 따라서 각자의 값을 가질 수 있다. 하지만 reference 타입의 변수들은 힙의 주소를 값으로 가지고 있기 때문에 변수명이 달라도 같은 객체를 가리키고 있기 때문에 생각한대로 수정할 수 없는 것이다.
![](images/2022-07-18-21-21-32.png)
![](images/2022-07-18-21-21-54.png)

const객체는 그 속성을 변경하는 것은 가능하지만 새로운 객체를 할당하는 것은 허용되지 않는다.
